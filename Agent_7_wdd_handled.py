...
class Agent_7_wdd_handled:
    ...
    def begin(arena):
        ...
        while game_count < number_of_games:
            ...
            prey = Prey()
            predator = Predator()
            agent7_wdd_handled = Agent_7_wdd_handled(prey.curr_pos, predator.curr_pos)
            ...
            while 1:
                ...
                print(agent7_wdd_handled.curr_pos, prey.curr_pos, predator.curr_pos)

                # Check if it knows where the predator is
                if max(agent7_wdd_handled.predator_belief_state.values()) == 1.0:
                    found_prey, node_surveyed = utils.survey_prey(agent7_wdd_handled, prey)
                    if found_prey:
                        if random.random() <= 0.1:
                            found_prey = False
                else:
                    found_predator, node_surveyed = utils.survey_predator(agent7_wdd_handled, predator)
                    if found_predator:
                        if random.random() <= 0.1:
                            found_predator = False

                # updating both belief states
                agent7_wdd_handled.prey_belief_state = utils.update_prey_belief_state_defective_drone(agent7_wdd_handled.prey_belief_state, \
                                                                          agent7_wdd_handled.curr_pos, \
                                                                          agent7_wdd_handled.prev_pos, \
                                                                          arena, \
                                                                          found_prey, \
                                                                          node_surveyed, \
                                                                          'after_survey')
                if max(agent7_wdd_handled.prey_belief_state.values()) == 1:
                    prey_certainty_counter += 1
                agent7_wdd_handled.predator_belief_state = utils.update_predator_belief_state_defective_drone(agent7_wdd_handled.predator_belief_state, \
                                                                                  agent7_wdd_handled.curr_pos, \
                                                                                  agent7_wdd_handled.prev_pos, \
                                                                                  arena, \
                                                                                  found_predator, \
                ...
                if agent7_wdd_handled.curr_pos == prey.curr_pos:
                    win_count += 1
                    break
                elif agent7_wdd_handled.curr_pos == predator.curr_pos:
                    loss_count += 1
                    break
                ...
                predator.distracted_move(agent7_wdd_handled.curr_pos, arena)
                ...
                # Checking termination states
                if agent7_wdd_handled.curr_pos == predator.curr_pos:
                    loss_count += 1
                    break
